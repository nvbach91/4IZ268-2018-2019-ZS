...
var pairs = [
  ...
  ['convince', 'přesědčit'],
  ['mediocre', 'nevalný'],
  ['walnuts', 'vlašské oříšky'],
  ['assign', 'přiřadit'],
  ['to bark at', 'štěkat na'],
  ['driving offence', 'dopravní přestupek']
];
var words = [];
for (var i = 0; i < pairs.length; i++) {
  words = words.concat(pairs[i])
}
...
//this is the game board and an array i.e. HTMLTokenList of cards
var gameField = document.getElementById("game-field");
var cards = gameField.getElementsByClassName("card");
function countRevealed() {
  var count = 0;
  for (var i = 0; i < cards.length; i++) {
    if (cards[i].className == "card revealed") {
      count++
    }
  }
  console.log(count)
